...
/*Converts a string as 'day','week', 'month' or a custom range into time_begin and time_end */

export function convertDateRange(s) {
  let res= {time_begin:  moment().format("YYYY-MM-DD 00:00:00") , time_end: moment().format("YYYY-MM-DD HH:mm:ss")};
  if(s == 'week'){
    res.time_begin = moment().subtract(7, 'days').format("YYYY-MM-DD 00:00:00");
  }else if( s == 'month'){
    res.time_begin = moment().subtract(1, 'months').format("YYYY-MM-DD 00:00:00");
  }else if(s.includes("custom")){
    res.time_begin = moment(s.substr(7,10), "YYYY-MM-DD").format("YYYY-MM-DD 00:00:00"); 
    res.time_end = moment(s.substr(18,10), "YYYY-MM-DD").format("YYYY-MM-DD 23:59:59"); 
  }
  return res;
};